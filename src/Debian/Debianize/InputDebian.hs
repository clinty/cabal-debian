...
import qualified Debian.Debianize.SourceDebDescription as S (binaryPackages, buildConflicts, buildConflictsIndep, buildDepends, buildDependsIndep, dmUploadAllowed, homepage, newSourceDebDescription', priority, section, SourceDebDescription, standardsVersion, uploaders, description, vcsFields, VersionControlSpec(VCSArch, VCSBrowser, VCSBzr, VCSCvs, VCSDarcs, VCSGit, VCSHg, VCSMtn, VCSSvn), XField(XField), xFields)
...
parseSourceDebDescription (Paragraph fields) binaryParagraphs =
    ...
    where
      ...
      readField (Field ("Description", value)) (desc, unrecognized) = (set S.description (Just (pack value)) desc, unrecognized)
